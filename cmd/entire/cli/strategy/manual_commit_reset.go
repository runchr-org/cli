...
// Output is written to w; callers should pass cmd.ErrOrStderr() or similar.
func (s *ManualCommitStrategy) Reset(ctx context.Context, w io.Writer) error {
	...
	for _, state := range sessions {
		if err := s.clearSessionState(ctx, state.SessionID); err != nil {
			fmt.Fprintf(w, "Warning: failed to clear session state for %s: %v\n", state.SessionID, err)
		} else {
...
// Output is written to w; callers should pass cmd.ErrOrStderr() or similar.
func (s *ManualCommitStrategy) ResetSession(ctx context.Context, w io.Writer, sessionID string) error {
	...
	if err := s.cleanupShadowBranchIfUnused(ctx, repo, shadowBranchName, sessionID); err != nil {
		fmt.Fprintf(w, "Warning: failed to clean up shadow branch %s: %v\n", shadowBranchName, err)
	} else {
